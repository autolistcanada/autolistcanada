...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  // Handle captured listings from content script
  if (msg.action === "listingCaptured") {
    // Store the captured listing in chrome storage
    const listingKey = `autolist_captured_${Date.now()}`;
    chrome.storage.local.set({[listingKey]: msg.listing}, () => {
      console.log("Listing captured and stored:", msg.listing);
      // Notify popup if it's open
      chrome.runtime.sendMessage({
        action: "listingCapturedNotification",
        listing: msg.listing,
        key: listingKey
      });
    });
